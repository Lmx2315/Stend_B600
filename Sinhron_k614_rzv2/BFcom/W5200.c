...
#define PF10_1  *pFIO_FLAG_D=*pFIO_FLAG_D|(PF10) // 
...
{
	//spi_cs(val);  
	 spi_cs_SPI(3u); //чипселект на PF3 
}
...
void loopback_udp(SOCKET s, uint16 port,unsigned char event)
{
	...
	put_str1("\r\n");
	itoa(s,strng1);
    put_str1(strng1);                                             /* if a socket is closed */
    put_str1(": Loop-Back UDP Started. port :");
	itoa(port,strng1);
    put_str1(strng1);
    put_str1("\r\n"); 
                             
    if(socket(s,Sn_MR_UDP,port,0x00)== 0)   {put_str1("\a : Fail to create socket.");		 /* reinitialize the socket */
	itoa(s,strng1);
    put_str1(strng1);
    put_str1("\r\n");};
		break;
  ...
  switch (getSn_SR(s))	{//   Get the state of socket s 
    case SOCK_ESTABLISHED :   // If the socket is established 	    
      if(ch_status[s] == ready_state) {  
        Transf("\r\nTdkvdELNET server established via SOCKET \r\n");
        snprintf(strng1," %d",s);
	    Transf(strng1);
...
void sendIAC(SOCKET s, uint8 r1, uint8 r2) 
{
  ...
  switch(r1) {
    case WILL :
      put_str1("sent : will");     // WILL command 
      break;
      
    case WONT :
      put_str1("sent : wont");      // WONT command 
      break;
      
    case DO :
      put_str1("sent : do");      //DO command 
      break;
      
    case DONT :
      put_str1("sent : dont");      // DONT command 
      break;  
