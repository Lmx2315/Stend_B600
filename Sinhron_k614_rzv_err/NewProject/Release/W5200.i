...
# 1 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 22 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"


# 30 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 82 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 100 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 127 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 146 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 197 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"

# 206 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"

# 223 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"

# 232 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"

# 249 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"

# 259 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 292 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 362 "C:\\Program Files (x86)\\Analog Devices\\VisualDSP 5.1.2\\Blackfin\\include\\stdint.h"
...
# 111 "..\\BFcom\\W5200.c"
  ...
  unsigned short sch_tx_packet=0;
...
unsigned char ch_status[2];
uint32_t presentTime;
unsigned short any_port = 1000;


unsigned char ch_status[2] = { 0,0 };	 
...
unsigned char IINCHIP_READ(unsigned short addr); 
unsigned char IINCHIP_WRITE(unsigned short addr,unsigned char data);
uint32_t presentTime;
uint32_t my_time;
...
unsigned char NUMBER_BLOK =1; 

unsigned char MAC[6]    ={0x64, 0xA2, 0x32, 0x01, 0x03,1};
unsigned char IP[4]     ={1, 3,  1,1+60};
unsigned char GateWay[4]={1, 3,  1, 254};
...
uint8_t 	 Hard_reg [6]={0xe0,0x69,0x95,0x04,0xdf,0x96};     
char		 Receive_DATA[16];
uint8_t		 Receive_IP[4]= {1, 3, 1, 151};
...
unsigned char  Dest_IP[4] = {1, 3, 1, 1}; 
unsigned short Dest_PORT = 23; 
...
unsigned char txsize[2] = {16,16};
unsigned char rxsize[2] = {16,16};
...
unsigned short getIINCHIP_RxMAX(unsigned char s);
unsigned short getIINCHIP_TxMAX(unsigned char s);
unsigned short getIINCHIP_RxMASK(unsigned char s);
unsigned short getIINCHIP_TxMASK(unsigned char s);
unsigned short getIINCHIP_RxBASE(unsigned char s);
unsigned short getIINCHIP_TxBASE(unsigned char s);
...
void setSn_MSS(SOCKET s, unsigned short Sn_MSSR0); 
void setSn_PROTO(SOCKET s, unsigned char proto); 
...
void send_data_processing(SOCKET s, unsigned char *wizdata, unsigned short len);
void recv_data_processing(SOCKET s, unsigned char *wizdata, unsigned short len);
void write_data(SOCKET s, vuint8 * src, vuint8 * dst, unsigned short len);
void read_data(SOCKET s, vuint8 * src, vuint8 * dst, unsigned short len);

void setGAR(unsigned char * addr); 
void saveSUBR(unsigned char * addr);	
void setSUBR(void); 
void clearSUBR(void);												
...
void close(SOCKET );
void sendIAC(SOCKET , unsigned char , unsigned char ); 
...
void loopback_tcps(SOCKET s, unsigned short port);
void loopback_tcpc(SOCKET s, unsigned short port);
void loopback_udp (SOCKET s, unsigned short port,unsigned char);



static unsigned char I_STATUS[2];
static unsigned short SMASK[2];  
static unsigned short RMASK[2];  
static unsigned short SSIZE[2];  
static unsigned short RSIZE[2];  
static unsigned short SBUFBASEADDRESS[2];  
static unsigned short RBUFBASEADDRESS[2];  
...
extern unsigned short sent_ptr;
...
void wait_10ms( unsigned short time_ms )
{
  register unsigned short i;
  for( i=0;i<time_ms;i++ )
  {
...
unsigned char windowfull_retry_cnt[2];
...
unsigned short pre_sent_ptr, sent_ptr;
...
unsigned char socket(SOCKET s, unsigned char protocol, unsigned short port, unsigned char flag)
{
	unsigned char ret;
	...
	if ((protocol == 0x01) || (protocol == 0x02) || (protocol == 0x03) || (protocol == 0x04) || (protocol == 0x05))
	{
	close(s);
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0000),protocol | flag);
		if (port != 0) {
			IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0004),(unsigned char)((port & 0xff00) >> 8));
			IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0004) + 1),(unsigned char)(port & 0x00ff));
		} else {
			local_port++; 
			IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0004),(unsigned char)((local_port & 0xff00) >> 8));
			IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0004) + 1),(unsigned char)(local_port & 0x00ff));
		}
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x01); 

		 
		while( IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0001)) ) 
			;
...
void close(SOCKET s)
{
	...
	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x10);
	...
	while( IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0001)) ) 
		...
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0002), 0xFF);
}
...
unsigned char listen(SOCKET s)
{
	...
	if (IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0003)) == 0x13)
	{
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x02);
...
unsigned char connect(SOCKET s, unsigned char * addr, unsigned short port)
{
	unsigned char ret;
...
put_str1("Fail[invalid ip,port]\r\n");	
	...
	else
	{
		...
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x000C),addr[0]);
		IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x000C) + 1),addr[1]);
		IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x000C) + 2),addr[2]);
		IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x000C) + 3),addr[3]);
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0010),(unsigned char)((port & 0xff00) >> 8));
		IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0010) + 1),(unsigned char)(port & 0x00ff));
		setSUBR();	 
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x04);
                 
		while ( IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0001)) ) ;
		while ( IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0003)) != 0x15 )
		{
			if(IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0003)) == 0x17)
			{
				break;
			}
			if (getSn_IR(s) & 0x08)
			{
				IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0002), (0x08));  
				ret = 0;
				break;
			}
		}
...
{
put_str1("disconnect()\r\n");
	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x08);
...
unsigned short send(SOCKET s, const unsigned char * buf, unsigned short len, _Bool retry)
{
	unsigned char status=0;
	unsigned short ret=0;
	unsigned short freesize=0;
	unsigned short txrd, txrd_before_send;
	...
	if(retry) ;
		...
		do 
		{
			...
			status = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0003));
			if ((status != 0x17) && (status != 0x1C))
			{
				ret = 0; 
				break;
			}
	...
	if(retry || ret != 0)
	{
		txrd_before_send = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0022));
		txrd_before_send = (txrd_before_send << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0022) + 1);
  
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x20);
	
		 
		while( IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0001)) );

		while ( (IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0002)) & 0x10) != 0x10 )
                {
			if(IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0002)) == 0x00)
			{
               	put_str1("SOCK_CLOSED.\r\n");  
				close(s);
				return 0;
			}
		}
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0002), 0x10);

		txrd = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0022));
		txrd = (txrd << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0022) + 1);
...
unsigned short recv(SOCKET s, unsigned char * buf, unsigned short len)
{
	unsigned short ret=0;
	...
	if ( len > 0u )
	{
		...
		IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0001),0x40);
...
unsigned short sendto(SOCKET s, const unsigned char * buf, unsigned short len, unsigned char * addr, unsigned short port)
{
	unsigned short ret=0;
   ...
   if (len > getIINCHIP_TxMAX(s)) ret = getIINCHIP_TxMAX(s); 
	...
	else
	{
	   ...
	   while ( (IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0002)) & 0x10) != 0x10 ) 
	   {
    	        if (IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0002)) & 0x08)
		{

                 
           	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0002), (0x10 | 0x08));  
		return 0;
		}
	   }
...
unsigned short recvfrom(SOCKET s, unsigned char * buf, unsigned short len, unsigned char * addr, unsigned short *port)
{
	...
	if ( len > 0u )
	{
   	ptr = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0028));
   	ptr = ((ptr & 0x00ff) << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0028) + 1);

   	switch (IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0000)) & 0x07)
   	{
   	case 0x02 :
   			read_data(s, (unsigned char *)ptr, head, 0x08);
   			ptr += 8;
			...
			read_data(s, (unsigned char *)ptr, buf, data_len); 
			ptr += data_len;

			IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0028),(unsigned char)((ptr & 0xff00) >> 8));
			IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0028) + 1),(unsigned char)(ptr & 0x00ff));
   			break;
   
   	case 0x03 :
   			read_data(s, (unsigned char *)ptr, head, 0x06);
   			ptr += 6;
			...
			IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0028) + 1),(unsigned char)(ptr & 0x00ff));
   			break;
   	case 0x04 :
   			read_data(s,(unsigned char*)ptr,head,2);
   			ptr+=2;
   			data_len = head[0];
...
unsigned char IINCHIP_WRITE(unsigned short addr,unsigned char data)
{
	...
	IINCHIP_SpiSendData(0x80);                  
	IINCHIP_SpiSendData(0x01);                  
...
unsigned short wiz_write_buf(unsigned short addr,unsigned char* buf,unsigned short len)
{
	...
	IINCHIP_SpiSendData((0x80 | ((len & 0x7F00) >> 8)));    
...
unsigned short wiz_read_buf(unsigned short addr, unsigned char* buf,unsigned short len)
{
	unsigned short idx = 0;
	...
	IINCHIP_SpiSendData(((addr+idx) & 0xFF00) >> 8);        
	IINCHIP_SpiSendData((addr+idx) & 0x00FF);               
	IINCHIP_SpiSendData((0x00 | ((len & 0x7F00) >> 8)));    
	IINCHIP_SpiSendData((len & 0x00FF));		        
...
void sysinit( unsigned char * tx_size, unsigned char * rx_size	)
{
	...
	SBUFBASEADDRESS[0] = (unsigned short)((0x0000 + 0x8000));		 
	RBUFBASEADDRESS[0] = (unsigned short)((0x0000 + 0xC000));		 

  for (i = 0 ; i < 2; i++)       
	{
    IINCHIP_WRITE((((0x0000 + 0x4000) + i * 0x0100 + 0x001F)),tx_size[i]);
    IINCHIP_WRITE((((0x0000 + 0x4000) + i * 0x0100 + 0x001E)),rx_size[i]);
		...
		if (ssum <= 16384)
		{
         switch( tx_size[i] )
			...
			case 1:
				SSIZE[i] = (short)(1024);
				SMASK[i] = (unsigned short)(0x03FF);
				break;
			case 2:
				SSIZE[i] = (short)(2048);
				SMASK[i] = (unsigned short)(0x07FF);
				break;
			case 4:
				SSIZE[i] = (short)(4096);
				SMASK[i] = (unsigned short)(0x0FFF);
				break;
			case 8:
				SSIZE[i] = (short)(8192);
				SMASK[i] = (unsigned short)(0x1FFF);
				break;
			case 16:
				SSIZE[i] = (short)(16384);
				SMASK[i] = (unsigned short)(0x3FFF);
			break;
			}
		}
		...
		if (rsum <= 16384)
		{
         switch( rx_size[i] )
			...
			case 1:
				RSIZE[i] = (short)(1024);
				RMASK[i] = (unsigned short)(0x03FF);
				break;
			case 2:
				RSIZE[i] = (short)(2048);
				RMASK[i] = (unsigned short)(0x07FF);
				break;
			case 4:
				RSIZE[i] = (short)(4096);
				RMASK[i] = (unsigned short)(0x0FFF);
				break;
			case 8:
				RSIZE[i] = (short)(8192);
				RMASK[i] = (unsigned short)(0x1FFF);
				break;
			case 16:
				RSIZE[i] = (short)(16384);
				RMASK[i] = (unsigned short)(0x3FFF);
				break;
			}
		}
		ssum += SSIZE[i];
...
{
	IINCHIP_WRITE(((0x0000 + 0x0001) + 0),addr[0]);
	IINCHIP_WRITE(((0x0000 + 0x0001) + 1),addr[1]);
	IINCHIP_WRITE(((0x0000 + 0x0001) + 2),addr[2]);
	IINCHIP_WRITE(((0x0000 + 0x0001) + 3),addr[3]);
}
...
void setSUBR(void)
{   
	IINCHIP_WRITE(((0x0000 + 0x0005) + 0), SUBNET[0]);
	IINCHIP_WRITE(((0x0000 + 0x0005) + 1), SUBNET[1]);
	IINCHIP_WRITE(((0x0000 + 0x0005) + 2), SUBNET[2]);
	IINCHIP_WRITE(((0x0000 + 0x0005) + 3), SUBNET[3]);
}

void clearSUBR(void)
{
	IINCHIP_WRITE(((0x0000 + 0x0005) + 0), 0);
	IINCHIP_WRITE(((0x0000 + 0x0005) + 1), 0);
	IINCHIP_WRITE(((0x0000 + 0x0005) + 2), 0);
	IINCHIP_WRITE(((0x0000 + 0x0005) + 3), 0);
}
...
{
	IINCHIP_WRITE(((0x0000 + 0x0009) + 0),addr[0]);
	IINCHIP_WRITE(((0x0000 + 0x0009) + 1),addr[1]);
	IINCHIP_WRITE(((0x0000 + 0x0009) + 2),addr[2]);
	IINCHIP_WRITE(((0x0000 + 0x0009) + 3),addr[3]);
	IINCHIP_WRITE(((0x0000 + 0x0009) + 4),addr[4]);
	IINCHIP_WRITE(((0x0000 + 0x0009) + 5),addr[5]);
}
...
{
	IINCHIP_WRITE(((0x0000 + 0x000F) + 0),addr[0]);
	IINCHIP_WRITE(((0x0000 + 0x000F) + 1),addr[1]);
	IINCHIP_WRITE(((0x0000 + 0x000F) + 2),addr[2]);
	IINCHIP_WRITE(((0x0000 + 0x000F) + 3),addr[3]);
}
...
void getGAR(unsigned char * addr)
{
	addr[0] = IINCHIP_READ((0x0000 + 0x0001));
	addr[1] = IINCHIP_READ((0x0000 + 0x0001)+1);
	addr[2] = IINCHIP_READ((0x0000 + 0x0001)+2);
	addr[3] = IINCHIP_READ((0x0000 + 0x0001)+3);
}
void getSUBR(unsigned char * addr)
{
	addr[0] = SUBNET[0];
...
void getSHAR(unsigned char * addr)
{
	addr[0] = IINCHIP_READ((0x0000 + 0x0009));
	addr[1] = IINCHIP_READ((0x0000 + 0x0009)+1);
	addr[2] = IINCHIP_READ((0x0000 + 0x0009)+2);
	addr[3] = IINCHIP_READ((0x0000 + 0x0009)+3);
	addr[4] = IINCHIP_READ((0x0000 + 0x0009)+4);
	addr[5] = IINCHIP_READ((0x0000 + 0x0009)+5);
}
void getSIPR(unsigned char * addr)
{
	addr[0] = IINCHIP_READ((0x0000 + 0x000F));
	addr[1] = IINCHIP_READ((0x0000 + 0x000F)+1);
	addr[2] = IINCHIP_READ((0x0000 + 0x000F)+2);
	addr[3] = IINCHIP_READ((0x0000 + 0x000F)+3);
}

void setMR(unsigned char val)
{
	IINCHIP_WRITE((0x0000 + 0x0000),val);
}
...
unsigned char getIR( void )
{
   return IINCHIP_READ((0x0000 + 0x0015));
}
...
void setRTR(unsigned short timeout)
{
	IINCHIP_WRITE((0x0000 + 0x0017),(unsigned char)((timeout & 0xff00) >> 8));
	IINCHIP_WRITE(((0x0000 + 0x0017) + 1),(unsigned char)(timeout & 0x00ff));
}
...
void setRCR(unsigned char retry)
{
	IINCHIP_WRITE((0x0000 + 0x0019),retry);
}
...
void setIMR(unsigned char mask)
{
	IINCHIP_WRITE((0x0000 + 0x0016),mask); 
}
...
void setSn_MSS(SOCKET s, unsigned short Sn_MSSR0)
{
	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0012),(unsigned char)((Sn_MSSR0 & 0xff00) >> 8));
	IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0012) + 1),(unsigned char)(Sn_MSSR0 & 0x00ff));
}

void setSn_TTL(SOCKET s, unsigned char ttl)
{
   IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0016), ttl);
}
...
void setSn_PROTO(SOCKET s, unsigned char proto)
{
	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0014),proto);
}
...
unsigned char getSn_IR(SOCKET s)
{
   return IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0002));
}
...
unsigned char getSn_SR(SOCKET s)
{
   return IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0003));
}
...
unsigned short getSn_TX_FSR(SOCKET s)
{
	...
	do
	{
		val1 = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0020));
		val1 = (val1 << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0020) + 1);
      if (val1 != 0)
		{
   			val = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0020));
   			val = (val << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0020) + 1);
		}
	} while (val != val1);
   return val;
}
...
unsigned short getSn_RX_RSR(SOCKET s)
{
	...
	do
	{
		val1 = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0026));
		val1 = (val1 << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0026) + 1);
      if(val1 != 0)
		{
   			val = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0026));
   			val = (val << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0026) + 1);
		}
	} while (val != val1);
   return val;
}
...
void send_data_processing(SOCKET s, unsigned char *data, unsigned short len)
{
	
	unsigned short ptr;
	ptr = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0024));
	ptr = (ptr << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0024) + 1);
	write_data(s, data, (unsigned char *)(ptr), len);
	ptr += len;

	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0024),(unsigned char)((ptr & 0xff00) >> 8));
	IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0024) + 1),(unsigned char)(ptr & 0x00ff));
...
void recv_data_processing(SOCKET s, unsigned char *data, unsigned short len)
{
	unsigned short ptr;
	ptr = IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0028));
	ptr = ((ptr & 0x00ff) << 8) + IINCHIP_READ(((0x0000 + 0x4000) + s * 0x0100 + 0x0028) + 1);
	...
	read_data(s, (unsigned char *)ptr, data, len); 
	ptr += len;
	IINCHIP_WRITE(((0x0000 + 0x4000) + s * 0x0100 + 0x0028),(unsigned char)((ptr & 0xff00) >> 8));
	IINCHIP_WRITE((((0x0000 + 0x4000) + s * 0x0100 + 0x0028) + 1),(unsigned char)(ptr & 0x00ff));
}
...
unsigned int destip_UDP    =0; 
unsigned short destport_UDP  =0;  
unsigned short FLAG_UDP_EVENT=0u;
unsigned char FLAG_UDP_INSTALL=0u;

void loopback_udp(SOCKET s, unsigned short port,unsigned char event)
{
	 short RSR_len;
	...
	case 0x22:
				...
				if ((FLAG_UDP_EVENT>0u)&&(FLAG_UDP_INSTALL==1u))
					{
						received_len=16+16+104; 

						sch_tx_packet++;
						
						if(sendto(s, TX_BUF, received_len,(unsigned char*)&destip_UDP, destport_UDP) == 0)	 
										{
											put_str1("\a\a\a : System Fatal Error.");
											itoa(s,strng1);
							                put_str1(strng1);
							                put_str1("\r\n");
										}
				...
				if ((RSR_len = getSn_RX_RSR(s)) > 0) 														 
					{
						if (RSR_len > 200) RSR_len = 200;							 
																												 
						received_len = recvfrom(s, RX_BUF, RSR_len, (unsigned char*)&destip_UDP, &destport_UDP);				 
						
						   	      lsr=(unsigned int) received_len;
								  lsr_razmer=lsr;
							      flag_RX_UDP=1;
							      FLAG_UDP_INSTALL=1;
							      sch_packet_UDP_reciv=sch_packet_UDP_reciv+1; 
	...
	itoa(s,strng1);
    put_str1(strng1);                                              
    put_str1(": Loop-Back UDP Started. port :");
	itoa(port,strng1);
    put_str1(strng1);
    put_str1("\r\n"); 
                             
    if(socket(s,0x02,port,0x00)== 0)   {put_str1("\a : Fail to create socket.");		  
	itoa(s,strng1);
    put_str1(strng1);
    put_str1("\r\n");};
		break;
...
void Set_network(void)
{
        ...
        put_str1("\r\n----------------------------------------- \r\n");         		
        put_str1("W5200E01-M3                       \r\n");        
        put_str1("Network Configuration Information \r\n");        
        put_str1("----------------------------------------- \r");  
        put_str1("\r\nMAC : ");
        
        getSIPR (tmp_array);
        put_str1("\r\nIP : ");
        itoa(tmp_array[0],strng1);
        put_str1(strng1);
	    put_str1(".");
	    itoa(tmp_array[1],strng1);
        put_str1(strng1);
	    put_str1(".");
	    itoa(tmp_array[2],strng1);
        put_str1(strng1);
	    put_str1(".");
	    itoa(tmp_array[3],strng1);
        put_str1(strng1);
...
void sendIAC(SOCKET s, unsigned char r1, unsigned char r2) 
{
